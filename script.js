...
async function initThree(){
  try{
    ...
    function render(ms=0){requestAnimationFrame(render);if(ms-last<33)return;const delta=Math.min((ms-last)/1000,.05);last=ms;const t=clock.getElapsedTime(),boost=Math.max(0,(boostUntil-ms)/900),entry=entryStart?Math.max(0,Math.min(1,(ms-entryStart)/1800)):0,ease=1-Math.pow(1-entry,3),x=1.75+(1-ease)*4.8+mx*.24,y=-.02+(1-ease)*.65-my*.12+Math.sin(boost*Math.PI)*.28,z=-.2+(1-ease)*-1.5+boost*.45,s=.78*userScale*(.35+.65*ease)*(1+Math.sin(boost*Math.PI)*.08);carRig.position.set(x,y+(!reduce?Math.sin(t*1.2)*.055:0),z);carRig.scale.setScalar(s);carRig.rotation.y=THREE.MathUtils.lerp(carRig.rotation.y,manualYaw+mx*.35+(autoRotate?t*.5:0)+(1-ease)*-1.15,.08);carRig.rotation.z=THREE.MathUtils.lerp(carRig.rotation.z,manualRoll+(x-previousX)*-.18+Math.sin(t*.7)*.014,.08);carRig.rotation.x=THREE.MathUtils.lerp(carRig.rotation.x,manualPitch-my*.14-boost*.12+(1-ease)*.12,.08);previousX=x;shadow.position.set(x,y-.62,z-.15);shadow.scale.set(1.9*s*(1+boost*.25),.34*s,1);shadow.material.opacity=.12*ease*(1-boost*.4);group.visible=true;group.rotation.y=t*.05;group.position.set(1.8,.1,-1.5);group.scale.setScalar(.58);if(mixer)mixer.update(delta);renderer.render(scene,camera)}
    render();
